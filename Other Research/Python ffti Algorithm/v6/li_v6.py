...
def inter_1D(coords, values, pos):
   ...
   Ni = len(coords)
   if pos < coords[0] or coords[Ni - 1] < pos:
      print("OUT OF RANGE")
      return "OUT OF RANGE"
   elif pos in coords:
      value_inter = values[list(coords).index(pos)]
   else:
      value_inter = interpolate(coords, values, pos)
...
def inter_2D(x_coords, y_coords, values2D, pos, order = 'xy'):
   ...
   if order == 'xy':
      ...
      value_inter = inter_1D(y_coords, values1D, pos[1])
   if order == 'yx':
      values1D = np.zeros(Ni)
      for i in range(0, Ni):
         values1D[i] = inter_1D(y_coords, values2D[i], pos[1])
      value_inter = inter_1D(x_coords, values1D, pos[0])
