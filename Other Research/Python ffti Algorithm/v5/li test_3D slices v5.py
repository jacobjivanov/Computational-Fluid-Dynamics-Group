...
x_pos = 1#2 * np.pi * np.random.rand()
...
for j in range(0, Nj_inter):
   for k in range(0, Nk_inter):
      x_slice_inter[j, k] = li.inter_3D(x, y, z, rho, pos = [x_pos, y_inter[j], z_inter[k]], order = 'xyz')
...
y_pos = 2# 2 * np.pi * np.random.rand()
...
for i in range(0, Ni_inter):
   for k in range(0, Nk_inter):
      y_slice_inter[i, k] = li.inter_3D(x, y, z, rho, pos = [x_inter[i], y_pos, z_inter[k]], order = 'xyz')
...
z_pos = 3#2 * np.pi * np.random.rand()
...
for i in range(0, Ni_inter):
   for j in range(0, Nj_inter):
      z_slice_inter[i, j] = li.inter_3D(x, y, z, rho, pos = [x_inter[i], y_inter[j], z_pos], order = 'xyz')
