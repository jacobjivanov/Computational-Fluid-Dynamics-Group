...
import numpy as np
import fourier_interpolate_v2 as fi
# import linear_interpolate as li

x_max = 10
y_max = 10

Ni = 51 # typically a round number + 1,
Nj = 51 # such that the interstitial values are nice
...
Ni_inter = 51 # typically a round number + 1,
Nj_inter = 51 # such that the interstitial values are nice
...
for j in range(0, Nj_inter):
   for i in range(0, Ni_inter):
      ...
      z_fi_approx[j][i] = fi.inter_2D(x, y, z, [x_inter[i], y_inter[j]])
      # z_fi_error[j][i] = z_fi_approx[j][i] - func(x_inter[i], y_inter[j])
...
ax.scatter(np.meshgrid(x, y)[0], np.meshgrid(x, y)[1], z, color = "grey", alpha = 0.2, label = "discrete input")

# Fourier Interpretation Surface Plot
ax.plot_surface(np.meshgrid(x_inter, y_inter)[0], np.meshgrid(x_inter, y_inter)[1], z_fi_approx, cmap = "RdBu", alpha = 0.8)
