...
def inter_2D(x_coords, y_coords, values2D, pos, *order):
   """
   ...
   Parameters: 
      ...
      order : String
         A string that gives the desired interpolation order. 
         Example: 'xy'
   """
   ...
   if order == 'xy': # interpolate at x first, then at y
      ...
      for j in range(0, Nj):
         values1D[j] = inter_1D(x_coords, values2D[j], pos[0])
   ...
   if order == 'yx': # interpolate at y first, then at x
      ...
      for i in range(0, Ni):
         values1D[i] = inter_1D(y_coords, values2D.transpose()[i], pos[1])
