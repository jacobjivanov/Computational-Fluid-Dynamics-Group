...
import numpy as np
import fourier_interpolate_v4 as fi
...
Ni = 79 # typically a round number + 1,
Nj = 105 # such that the interstitial values are nice
...
z_fi_approx = np.zeros(shape = (Ni_inter, Nj_inter))
z_fi_error = np.zeros(shape = (Ni_inter, Nj_inter))

# PART OF TESTING, IGNORE
# z_fi_order_diff = np.zeros(shape = (Ni_inter, Nj_inter))
...
for i in range(0, Ni_inter):
   for j in range(0, Nj_inter):
      z_fi_approx[i, j] = fi.inter_2D(x, y, z, [x_inter[i], y_inter[j]])
      
      z_fi_error[i][j] = z_fi_approx[i][j] - func(x_inter[i], y_inter[j])

      # PART OF TESTING, IGNORE
      # z_fi_order_diff[i][j] = fi.inter_2D(x, y, z, [x_inter[i][0], y_inter[0][j]], order = 'xy') - fi.inter_2D(x, y, z, [x_inter[i][0], y_inter[0][j]], order = 'yx')
...
if PLOT_STYLE == "3D SURFACE":
   ...
   ax.plot_surface(x_inter, y_inter, z_fi_approx, cmap = "cool", alpha = 0.8)
...
if PLOT_STYLE == "2D CONTOUR":
   ...
   plt.contourf(np.meshgrid(x_inter, y_inter, indexing = 'ij')[0], np.meshgrid(x_inter, y_inter, indexing = 'ij')[1], z_fi_approx, cmap = 'jet', levels = 100)
   plt.colorbar()
   plt.xlabel('x-axis')
   plt.ylabel('y-axis')
   ...
   plt.clf() # clears previous
   plt.contourf(np.meshgrid(x_inter, y_inter, indexing = 'ij')[0], np.meshgrid(x_inter, y_inter, indexing = 'ij')[1], z_fi_error, cmap = 'jet', levels = 100)
   plt.colorbar()
   plt.xlabel('x-axis')
   plt.ylabel('y-axis')
   ...
   # "on domain 2 pi x 2 pi plot the logarithm of absolute error between your interpolated function and the exact, this is a plot of the log (abs (error))" 
   plt.clf() # clears previous
   
   error = np.abs(z_fi_error)
   for i in range(0, Ni_inter):
      for j in range(0, Nj_inter):
         if error[i, j] == 0:
            error[i, j] = 0.0000000001

   plt.contourf(np.meshgrid(x_inter, y_inter, indexing = 'ij')[0], np.meshgrid(x_inter, y_inter, indexing = 'ij')[1], np.log(error), cmap = 'jet', levels = 100)
   plt.colorbar()
   plt.xlabel('x-axis')
   plt.ylabel('y-axis')
   plt.title("Log Absolute Error Between Interpolation of " + r"$e^{\sin(x) + \cos(y)}$" + "\non Ni_inter: {0}, Nj_inter: {1} and Exact Solution".format(Ni_inter, Nj_inter))
   plt.savefig("BULLET 4.png", dpi = 600)
