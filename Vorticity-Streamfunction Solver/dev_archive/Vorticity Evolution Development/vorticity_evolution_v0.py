...
while t < t_end:
    if n % 100 == 0: # plot procedure within
        ...
        omega = np.real(ifft2(Omega))
        psi = np.real(ifft2(Psi))
        dil = np.real(recover_dil(u, v))

        uv_num = np.empty(shape = (M, N))
        for i in range(0, M):
            for j in range(0, N):
                uv_num[i, j] = (u[i, j] ** 2 + v[i, j] ** 2) ** (1/2)
        ...
        if plot_version == "diff":
            ...
            vel_plot = ax[0, 0].pcolormesh(x, y, np.transpose(uv_num - uv_ana), cmap = 'coolwarm')
        ...
        if plot_version == "num":
            ...
            vel_plot = ax[0, 0].pcolormesh(x, y, np.transpose(uv_num), cmap = 'coolwarm')
