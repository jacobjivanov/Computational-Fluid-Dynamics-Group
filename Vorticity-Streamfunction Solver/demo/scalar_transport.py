...
current_file = np.load("demo/Omega+Theta_p+t n = 20740.npz")
...
k_p, k_q, k_U, k_V = wavearrays(512, 512)
...
for p in range(0, 2):
    ...
    ax[p].set_xlabel(r"$x$")
    ax[p].xaxis.set_major_formatter(FuncFormatter(lambda val,pos: r'{:.0g}$\pi$'.format(val/np.pi) if val !=0 else '0'))
    ax[p].yaxis.set_major_formatter(FuncFormatter(lambda val,pos: r'{:.0g}$\pi$'.format(val/np.pi) if val !=0 else '0'))
...
fig.suptitle("t = {0:.5f}".format(t), y = 0.9)
plt.savefig("scalar_transport.png", dpi = 200)